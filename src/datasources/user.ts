import { Pet, PrismaClient, Trip } from "@prisma/client";
...
export class UserAPI extends DataSource {
  ...
  async bookTrips({ launchIds }: { launchIds: number[] }): Promise<Trip[] | undefined> {
  ...
  async bookTrip({ launchId }: { launchId: number }): Promise<Trip | Trip> {
    const userId = this.context.user.id;
